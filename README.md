...
|---|---|---|---|---|
| 프레임워크 | `react` / `react-dom` | **19.2.1** | 19.2.7 | 업데이트 권장 |
| 언어 | `typescript` | **5.6.3** | 6.0.3 | v6 breaking change 검토 후 적용 |
| 스타일링 | `tailwindcss` | **4.1.14** | 4.3.2 | 업데이트 권장 |
| 빌드 도구 | `vite` | **7.1.9** | 8.1.2 | v8 Rolldown 번들러 전환 — 마이그레이션 필요 |
| UI 컴포넌트 | `shadcn/ui` + `@radix-ui` | **radix 1.1.15** | — | shadcn/ui는 별도 버전 없음 |
| 아이콘 | `lucide-react` | **0.453.0** | 1.22.0 | 업데이트 권장 |
| 라우팅 | `wouter` | **3.7.1** | 3.10.0 | 업데이트 권장 |
| 유틸리티 | `clsx` / `class-variance-authority` | 2.1.1 / 0.7.1 | — | 안정 버전 유지 |
| 차트 | 순수 SVG | — | — | 외부 라이브러리 없음 |

> **[확인필요]** TypeScript 6.0은 일부 breaking change가 있습니다. Vite 8.0은 Rolldown(Rust 기반 번들러)으로 전환되어 마이그레이션 가이드 검토 후 적용을 권장합니다.
